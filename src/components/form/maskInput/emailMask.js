...
export default function emailMask(rawValue, config) {
    ...
    const indexOfFirstAtSymbol = rawValue.indexOf(this.atSymbol)
    const indexOfLastDot = rawValue.lastIndexOf(this.dot)
    const indexOfTopLevelDomainDot = (indexOfLastDot < indexOfFirstAtSymbol) ? -1 : indexOfLastDot

    let localPartToDomainConnector = getConnector(rawValue, indexOfFirstAtSymbol + 1, this.atSymbol)
    let domainNameToTopLevelDomainConnector = getConnector(rawValue, indexOfTopLevelDomainDot - 1, this.dot)
