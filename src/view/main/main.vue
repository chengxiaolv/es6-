...
import { getNewTagList, getNextTag } from '@/libs/util'
...
export default {
    ...
    methods: {
        ...
        handleCloseTag(res, type, name) {
            const nextTag = getNextTag(this.tagNavList, name)
            this.setTagNavList(res)
            if (type === 'all') this.turnToPage('home')
            else if (this.$route.name === name) this.$router.push({ path: nextTag.path })
        },
        handleClick(item) {
            this.$router.push({
                path: item.path,
