			...
			<Row>
				<Col span="12">
					<FormItem label="客户姓名:" prop="customerId">
						<lookup v-model="customerIdMask" placeholder="客户姓名" title="客户查询" :modalHeight="480" :readonly="true">
			...
			<Row>
				<Col span="12">
					<FormItem label="申请金额:" prop="applyAmount">
						<Input v-model="mInfo.applyAmount" placeholder="申请金额" :maxlength="32"></Input>
					</FormItem>
				</Col>
				<Col span="12">
					<FormItem label="申请期限:" prop="applyTerm">
						<Input v-model="mInfo.applyTerm" placeholder="申请期限" :maxlength="512">
			...
			<Row>
				<Col span="12">
					<FormItem label="申请利率:" prop="applyRate">
						<Input v-model="mInfo.applyRate" placeholder="申请利率" :maxlength="16"></Input>
					</FormItem>
				</Col>
				<Col span="12">
					<FormItem label="逾期利率:" prop="overdueRate">
						<Input v-model="mInfo.overdueRate" placeholder="逾期利率" :maxlength="16"></Input>
					</FormItem>
				</Col>
			</Row>
			<Row>
				<Col span="12">
					<FormItem label="逾期利率上浮:" prop="overdueRateFlow">
						<Input v-model="mInfo.overdueRateFlow" placeholder="逾期利率上浮" :maxlength="16"></Input>
					</FormItem>
				</Col>
				<Col span="12">
					<FormItem label="复利利率上浮:" prop="repeatRateFlow">
						<Input v-model="mInfo.repeatRateFlow" placeholder="复利利率上浮" :maxlength="16"></Input>
					</FormItem>
				</Col>
			</Row>
			<Row>
			...
			<Row>
				<Col span="12">
					<FormItem label="登记时间:" prop="regDate">
						<DatePicker v-model="regDateMask" type="date" placeholder="登记时间" @on-change="onRegDateChg"></DatePicker>
					</FormItem>
				</Col>
				<Col span="12">
...
<script>
import { materialsUpdate, materialsInfo} from '@/api/materials'
...
export default {
	...
	computed:{
		...
		regDateMask:{
			get(){
				return this.mInfo.regDate ? new Date(this.mInfo.regDate) : ''
			},
			set(val){
				this.mInfo.regDate = ymd(val)
			}
		},
		productTypeList(){
	...
	methods: {
		fetch(){
			return materialsInfo(this.id).then(res => {
				...
				this.optrUser.push({ text: res.data.optrUserName, value: this.mInfo.optrUserId })
   	     	})
		},
		getCustomerName(){
